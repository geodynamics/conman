...
echo '\hline ' >> $TEXFILE
  ...
  blockmean temp.xyz -R0/660/0/600 -I10.0/10.0 > tmp
  surface tmp -Gtemp.grd -R0/660/0/600 -I10.0/10.0
  t11=`grdtrack ../graphics/t11.xy -Gtemp.grd | awk '{printf "%5.2f", $3}'`
  tos=`grdtrack ../graphics/top-slab.xy -Gtemp.grd | awk '{s+=$3*$3} END {printf "%5.2f", sqrt(s/36)}'`
  twedge=`grdtrack ../graphics/wedge.xy -Gtemp.grd | awk '{s+=$3*$3} END {printf "%5.2f", sqrt(s/78)}'`
